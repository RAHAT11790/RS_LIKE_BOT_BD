    ...
    try:
        if server_name == "IND":
            with open("token_ind.json", "r") as f:
                tokens = json.load(f)
                if not tokens:
                    raise ValueError("token_ind.json is empty")
                return tokens
        elif server_name in {"BR", "US", "SAC", "NA"}:
            with open("token_br.json", "r") as f:
                tokens = json.load(f)
                if not tokens:
                    raise ValueError("token_br.json is empty")
                return tokens
        else:  # BD and other servers
            with open("token_bd.json", "r") as f:
                tokens = json.load(f)
                if not tokens:
                    raise ValueError("token_bd.json is empty")
                return tokens
    except FileNotFoundError:
        raise FileNotFoundError(f"Token file for {server_name} not found")
    except json.JSONDecodeError:
        raise ValueError(f"Invalid JSON in token file for {server_name}")
    except Exception as e:
        raise Exception(f"Error loading tokens for {server_name}: {str(e)}")
...
def handle_requests():
    ...
    def process_request():
        try:
            ...
            return {
                "error": f"Token file not found for server {server_name}",
                "status": 500
            }
        except Exception as e:
            return {
                "error": f"Error loading tokens for server {server_name}: {str(e)}",
                "status": 500
            }
            
        if not data:
            return {
                "error": f"No tokens available for server {server_name}",
                "status": 500
            }
        ...
        if before is None:
            return {
                "error": "Failed to fetch initial like count",
                "status": 500
            }
        ...
        if after is None:
            return {
                "error": "Failed to fetch like count after sending requests",
                "status": 500
            }
